...
do_install() {
	if ${@bb.utils.contains("GHRD_TYPE", "pr", "true", "false", d)}; then
		...
		install -m 0644 ${WORKDIR}/hardware/precompiled/rbf/achilles-${SOM_REV}-${SOM_VER}/blink_led_default.pr_partition.rbf ${D}${base_libdir}/firmware
		install -m 0644 ${WORKDIR}/hardware/precompiled/rbf/achilles-${SOM_REV}-${SOM_VER}/blink_led_fast.pr_partition.rbf ${D}${base_libdir}/firmware
		install -m 0644 ${WORKDIR}/hardware/precompiled/rbf/achilles-${SOM_REV}-${SOM_VER}/blink_led_slow.pr_partition.rbf ${D}${base_libdir}/firmware
...
do_deploy() {
	...
	install -m 0644 ${WORKDIR}/hardware/precompiled/hps_isw_handoff/achilles-${SOM_REV}-${SOM_VER}/hps.xml ${DEPLOY_DIR_IMAGE}/hps.xml
	install -m 0644 ${WORKDIR}/hardware/precompiled/rbf/achilles-${SOM_REV}-${SOM_VER}/achilles_${SOM_REV}_${SOM_VER}_ghrd.periph.rbf ${DEPLOY_DIR_IMAGE}
	install -m 0644 ${WORKDIR}/hardware/precompiled/rbf/achilles-${SOM_REV}-${SOM_VER}/achilles_${SOM_REV}_${SOM_VER}_ghrd.core.rbf ${DEPLOY_DIR_IMAGE}
}
